...
def get_gender(audio_path: str) -> int:
    ...
    signal = resampler(audio)
    if len(signal[0]) < 16000:
        print(f"Audio too short: {audio_path}, {len(signal[0])}")
        signal = torch.cat((signal[0], signal[0],signal[0],signal[0],signal[0])).unsqueeze(0)
        
    return process_func(signal, resample_rate)[0][1:3].argmax()
# process_func(signal, resample_rate)[0][1:3].argmax()
